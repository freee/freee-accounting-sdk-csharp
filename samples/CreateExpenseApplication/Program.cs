...
namespace CreateExpenseApplication
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            // 経費精算を作成する
            await new ExpenseApplicationsApi(config).CreateExpenseApplicationAsync(new ExpenseApplicationCreateParams
            {
                CompanyId = companyId,
                Title = "テスト経費精算",
                Description = "これは SDK のテストで作成したデータです",
                IssueDate = "2019-07-22",
                ExpenseApplicationLines = new List<ExpenseApplicationCreateParamsExpenseApplicationLines>
                {
                    new ExpenseApplicationCreateParamsExpenseApplicationLines
                    {
